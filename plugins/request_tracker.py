...
rchats = {}
# requestGroup = requestChannel
rchats[-1001506862275] = -1001651793041  
...
async def _requests(client, update):
    ...
    buttons_tosend.append(
        [
            InlineKeyboardButton("Done", r"reqs_completed"),
            InlineKeyboardButton("Reject", r"reqs_rejected"),
        ]
    )
    buttons_tosend.append(
        [
    ...
    rmsg = await app.send_message(
        chat_tosend, text_tosend, reply_markup=InlineKeyboardMarkup(buttons_tosend)
    )
    ...
    await app.send_message(
        ...
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("View Status", url=rmsg.link)]]
        ),
...
async def cb_requests(client, update):
    ...
    chat = None
    for g, c in rchats.items():
      if c == message.chat.id:
        chat = await app.get_chat(g)
        break 
    tosend = f"<i><u><b>[{chat.title}]:</i></u></b>\n<i>Your request for '{crequest}' is {action}.</i>"
    toedit = f"<b>▼{action.upper()}▼</b>\n\n<s>{message_text}</s>"

    try:
        await app.send_message(user_id, tosend)
    except BaseException:
        pass

    await message.edit_text(toedit, reply_markup=None)
